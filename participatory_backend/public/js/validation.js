...
engage.ui.Validation = class {
  ...
  make() {
    this.filter_edit_area = $(
      frappe.render_template("edit_validation", {
        conditions: this.conditions,
      })
    );
    this.parent &&
      this.filter_edit_area.appendTo(this.parent.find(".filter-edit-area"));
    this.make_select();
    this.set_events();
    this.setup();
  }
  ...
  set_values(doctype, fieldname, condition, value, error_msg) {
    ...
    if (error_msg) this.set_error_message(error_msg, true);
  ...
  get_error_message() {
    return this.filter_edit_area.find(".error-message").val();
  }
  ...
  set_error_message(error_msg, trigger_change = false) {
    let $error_msg_field = this.filter_edit_area.find(".error-message");
    $error_msg_field.val(error_msg);
    if (trigger_change) $error_msg_field.change();
  }

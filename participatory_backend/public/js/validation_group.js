frappe.provide("engage.ui");
engage.ui.ValidationGroup = class {
  constructor(opts) {
    $.extend(this, opts);
  ...
  update_filter_button() {
    ...
    this.filter_button.attr(
      "title",
      `${this.filters.length} Filter${
        this.filters.length > 1 ? "s" : ""
      } Applied`
    );
  }
  ...
  set_filter_events() {
    this.wrapper.find(".add-filter").on("click", () => {
      this.toggle_empty_filters(false);
      this.add_filter(this.doctype, this.fieldname /*"name"*/);
    });
  ...
  add_filter(doctype, fieldname, condition, value, error_msg, hidden) {
    ...
    if (is_new_filter && this.wrapper.find(".new-filter:visible").length) {
      ...
      let args = [doctype, fieldname, condition, value, error_msg, hidden];
  ...
  _push_new_filter(
    ...
    let filter = new engage.ui.Validation(args);
  ...
  get_filter_area_template() {
    return $(`
			<div class="filter-area">
				<div class="row">
					<div class="col-sm-5 text-muted">${__("Operator")}</div>
					<div class="col-sm-6 text-muted">${__("Value")}</div>
          <!--Hide errors field as instead of validation level errors, we will have one error message for all validations-->
					<div class="col-sm-4 text-muted d-none">${__("Error Message")}</div> 
				</div>
				<div class="filter-edit-area">
					<div class="text-muted empty-filters text-center">
						${__("No validations selected")}
					</div>
				</div>
				<hr class="divider"></hr>

...
WEB_FORM_LINK_FIELD_DOC_FILTER_PATTERN = re.compile(
    r"(\"|\')(web_form_values\..*?)(\"|\')"
)
...
REPLACE_READONLY_FIELDTYPE_WITH_ORIGINAL_FIELDTYPE = True
...
class FieldFilter:
    ...
    # filters_plain: str
    filters_parsed: list
    depends_on_form_field_value: (
        bool  # does the filter depend on values specified in the current form_field
    )
...
class EngagementForm(Document):
    ...
    def validate(self):
        ...
        if self.is_new() or not self.form_key:
            # generate a key to uniquely identify a form. It helps especially to prefix dynamic
            # form child tables generated by the engagement form. See handle_child_table_read_only_field method
            key = "".join(
        ...
        self.route = (
            self.get_route()
        )  # self.web_title.lower().replace(" ", "-") if self.web_title else None
    ...
    def generate_image_fields(self):
        if not self.field_is_table:
            ...
            self.qr_code = get_qrcode(
                self.form_url, form_image
            )  # if form_image else None
    ...
    def get_form_image(self):
        ...
        if logo_files:
            print(logo_files)
            print(frappe.local.sites_path)
            return frappe.utils.get_files_path(
                logo_files[0].file_name, is_private=logo_files[0].is_private
            )
        return None
    ...
    def after_rename(self, old_name, new_name, merge=False):
        ...
        try:
            self.web_title = (
                new_name  # set this to ensure that web forms are generated also
            )
            # self.validate()
    ...
    def on_trash(self):
        ...
        if doctype:
            ...
            frappe.delete_doc(
                "DocType", doctype, ignore_permissions=True, delete_permanently=True
            )
    ...
    def set_form_title_field(self):
        ...
        if fld:
            ...
            if cint(self.show_title_field_in_link):
                frappe.db.set_value(
                    "DocType", self.name, "title_field", fld[0].field_name
                )
                frappe.db.set_value(
        ...
        for fld in self.form_fields:
            ...
            if fld.field_type in ["Link", "Table MultiSelect"]:
                ...
                if not fld.field_doctype:
                    frappe.throw(
                        _(
                            "Row {0}. You must specify the Form for field {1}".format(
                                fld.idx, fld.field_label
                            )
                        )
                    )
            ...
            if fld.field_type in ["Table"]:
                if not fld.field_child_doctype:
                    frappe.throw(
                        _(
                            "Row {0}. You must specify the Child Form for field {1}".format(
                                fld.idx, fld.field_label
                            )
                        )
                    )
            if fld.field_type == "Select":
                if not fld.field_choices.strip():
                    frappe.throw(
                        _(
                            "Row {0}. You must specify the choices for field {1}".format(
                                fld.idx, fld.field_label
                            )
                        )
                    )
            ...
            if fld.field_type == "Linked Field":
                ...
                field.parent_options = parent_field_options or ""
    ...
    def validate_linked_field(self, field):
        ...
        if not frappe.db.exists("DocType", form):
            frappe.throw(
                _(
                    "Row {0}. The specified linked form {1} does not exist".format(
                        field.idx, linked_form
                    )
                )
            )
        ...
        if not [x for x in form.fields if x.fieldname == docfield]:
            frappe.throw(
                _(
                    "Row {0}. The specified linked form propery {1} does not exist in form {2}".format(
                        field.idx, field.linked_form_property, linked_form
                    )
                )
            )
    ...
    def make_doctype(self):
        ...
        # set search fields
        search_fields = ",".join(
            [x.field_name for x in self.form_fields if x.field_is_search_field]
        )
...
for i, itm in enumerate(doc.{field.field_name}):
    ...
    def sanitize_filters(self, filters):
        ...
        matches = LINK_FIELD_DOC_FILTER_PATTERN.findall(
            filters
        )  # returns list of tuples as [('"', 'doc.admin_1', '"'), ('"', 'doc.admin_2', '"')]
    ...
    def make_web_form_on_change_link_function(
        self, field: EngagementFormField
    ):  # , filter_map: list[type[FieldFilter]]):
        """
        ...
        for filter in parsed_filters:
            if (
                str(filter[3]).find(DOC_PREFIX_FORMULA) == -1
            ):  # there is no occurrence of doc.
                fltr = FieldFilter()
                fltr.target_field = field.field_name
    ...
    def make_client_script(self):
        ...
        filter_fields = [
            x
            for x in self.form_fields
            if x.field_filters_plain and x.field_type in ALLOWED_FILTER_FIELD_TYPES
        ]
    ...
    def _get_docfield(self, form_field: EngagementFormField):
        def _get_options():
            ...
            if form_field.field_type in ["Select", "Select Multiple"]:
                ...
                if form_field.field_choices.strip() == "":
                    frappe.throw(
                        _(
                            "Field {} in row {}. You must specify the choices for the Select Field".format(
                                form_field.field_label, form_field.idx
                            )
                        )
                    )
        ...
        def _get_fetch_from(field: EngagementFormField):
            if field.field_type == "Linked Field":
                return self.get_linked_field_fetch_from(field)
            return None
        ...
        # set depends on
        form_field.depends_on = _set_depends_on(
        ...
        field = {
            "doctype": "DocField",
            "label": (
                form_field.field_label.strip() if form_field.field_label else ""
            ),  # if form_field.field_type not in ['Column Break'] else '',
            "fieldname": self.get_field_name(form_field),
            "fieldtype": _get_field_type(form_field),
    ...
    def is_linked_field_a_child_table(
        ...
        parent_field = [
            x for x in self.form_fields if x.field_name == field.linked_form
        ]
        parent_field = parent_field[0]
        form = frappe.get_doc("DocType", parent_field.field_doctype)
        # check if the linked field in the form is of type Table
        docfield = [x for x in form.fields if x.fieldname == field.linked_form_property]
        docfield = docfield[0]
        return (
            docfield.fieldtype == "Table",
            parent_field.field_name,
            parent_field.field_doctype,
    ...
    def get_linked_field_field_type(self, field):
        """
        Get the field type of a linked field. When the type of the linked field is a Child Table,
        it returns Table else it returns Read Only
        """
        (
            is_table,
            trigger_field_name,
    ...
    def get_linked_field_fetch_from(self, field):
        """
        Get the value of fetch_from. Consider that the field could be linked to
        a Child Table
        """
        (
            is_table,
    ...
    def make_ref_doctype_name(self, form_field, max_length=DOCTYPE_MAX_LENGTH):
        ...
        if len(name) > max_length:
            if len(name) > max_length:
                ...
                form_initials = "".join(
                    [x[0] for x in self.form_name.replace("  ", " ").split(" ")]
                ).upper()
    ...
    def make_child_doctype_name(self, form_field):
        ...
        """
        reference_doctype_name = self.make_ref_doctype_name(
            form_field, max_length=DOCTYPE_MAX_LENGTH - len(" Item")
        )
    ...
    def handle_child_table_read_only_field(self, field: EngagementFormField):
        ...
        def _make_child_doctype(source_doctype: str, target_doctype: str):
            def _set_field(fld):
                existing_indices = [
                    ...
                    if item.fieldname == fld.fieldname
                ]
            ...
            def _validate_additional_fields():
                ...
                if field.additional_linked_table_fields:
                    for i, fld in enumerate(
                        list(json.loads(field.additional_linked_table_fields))
                    ):
                        if fld["fieldtype"] == "Link":
                            val = frappe.db.get_value(
                                "DocType",
                                {"name": fld["options"].strip()},
                                ["name", "istable"],
                                as_dict=True,
                            )
                            if not val:
                                frappe.throw(
                                    f"Row {field.idx}. The form specified [{fld['options']}] in additional fields row {i+1} does not exist"
                                )
                            if val.istable == True:
                                frappe.throw(
                                    f"Row {field.idx}. The form specified [{fld['options']}] in additional fields row {i+1} is a Child Table. You can only link Main Forms"
                                )
                        if fld["fieldtype"] == "Select":
                            ...
                            if choices.strip() == "":
                                frappe.throw(
                                    f"Row {field.idx}. You must specify the choices for the additional Select Field [{i+1}]"
                                )
            ...
            if not source.istable:
                frappe.throw(
                    "Row {0}. {1}. The specified linked field property is not a child form".format(
                        field.idx, field.field_label
                    )
                )
            ...
            for fld in source.fields:
                ...
                del fld_dict["name"]
                del fld_dict["parentfield"]
                del fld_dict["parenttype"]
                del fld_dict["parent"]
                del fld_dict["creation"]
                del fld_dict["modified"]
                fld_dict["reqd"] = (
                    False  # do this to avoid a situation where data will not be saved if some fields had been earlier missed
                )
                fld_dict["columns"] = (
                    2  # set columns as 2 to try maximize on having more columns visible
                )
                _set_field(fld_dict)
            ...
            if field.additional_linked_table_fields:
                ...
                for col in list(json.loads(field.additional_linked_table_fields)):
                    if "fieldtype" not in col or not col["fieldtype"]:
                        frappe.throw(
                            _(
                                f'You must specify the field type for additional field {col["idx"]}'
                            )
                        )
                    if not col["label"] and not col["fieldname"]:
                        frappe.throw(
                            _(
                                f'You must specify either the label or the field id for additional field for {col["idx"]}'
                            )
                        )
                    del col["idx"]
                    del col["name"]
                    del col["__islocal"]
                    col["fieldname"] = (
                        col["fieldname"] if "fieldname" in col else scrub(col["label"])
                    )
                    col["columns"] = (
                        2  # set columns as 2 to try maximize on having more columns visible
                    )
                    col["in_list_view"] = (
                        True  # make additional fields visible in Grid View by default
                    )
        ...
        def _make_client_script(
            trigger_field: str,
            trigger_field_doctype: str,
            trigger_field_table_property: str,
            target_field: str,
        ):
            self.child_table_filter_scripts[trigger_field] = f"""{NEWLINE}
    ...
    def handle_multi_select(self, form_field, select_type):
        ...
        def handle_as_normal_table_field():
            ...
            child_doc = (
                ...
                if not child_doctype_exists
                else frappe.get_doc("DocType", child_doctype_name)
            )
            ...
            field = {
                ...
                "label": (
                    ...
                    if form_field.field_type not in ["Column Break"]
                    else ""
                ),
                "fieldname": self.get_field_name(form_field),
                "fieldtype": "Table",
        ...
        def handle_with_doctypes():
            ...
            if select_type == SELECT_MULTIPLE:
                ...
                ref_doc = (
                    frappe.new_doc("DocType")
                    if deleted
                    else frappe.get_doc("DocType", reference_doctype_name)
                )
                ref_doc.fields = []
                ref_doc.permissions = []
                ...
                self._set_roles(
                    ref_doc
                )  # append same permissions as the form being designed
                ...
                for opt in options:
                    ...
                    if not frappe.db.exists(reference_doctype_name, opt):
                        ...
                        frappe.get_doc(
                            {"doctype": reference_doctype_name, OPTION_NAME_FIELD: opt}
                        ).insert(ignore_permissions=True)
            elif select_type == TABLE_MULTISELECT:
    ...
    def cleanup_multiselect(self):
        ...
        ):  # when renaming, the value of self._doc_before_save is None
            exists = [
                x
                for x in self._doc_before_save.form_fields
                if x.field_type in ["Table MultiSelect", "Select Multiple"]
            ]
            for fld in exists:
    ...
    def delete_doctype(self, doctype):
        ...
        for itm in lst:
            ...
            links = get_linked_docs(
                doctype, itm.name, linkinfo=get_linked_doctypes(doctype)
            )
            if not links:
                frappe.delete_doc(
                    doctype, itm.name, ignore_permissions=True, delete_permanently=True
                )
            else:
                error = True
    ...
    def get_route(self, fqdn=False):
        ...
        route = (
            scrub(self.name).replace("_", "-").strip("-")
        )  # if not self.route else self.route
        if route and fqdn:
            route = get_url(route)
        return route
    ...
    def publish_form(self):
        ...
        def _make_web_form_script():
            ...
            def _make_readonly_trigger(source_field: str):
                ...
                """
                readonly_fields = [
                    frappe._dict(x.__dict__)
                    for x in self.read_only_fields_map
                    if x.parent_field == source_field
                ]
                script = ""
                if readonly_fields:
                    ...
                    target_child_table_grid_doctype = readonly_fields[
                        0
                    ].target_child_table_doctype
            ...
            def _make_readonly_trigger_for_child_tables(source_field: str):
                """
                Check if there are other fields that should reset their values when the value of this field changes
                """
                child_table_readonly_fields = [
                    frappe._dict(x.__dict__)
                    for x in self.read_only_fields_map
                    if x.parent_field == source_field and x.is_target_a_child_table
                ]
                script = ""
                if child_table_readonly_fields:
                    target_field = child_table_readonly_fields[0].target_field
                    doctype = child_table_readonly_fields[0].parent_doctype
                    parent_field = child_table_readonly_fields[0].parent_field
                    parent_fields = [
                        x.parent_property for x in child_table_readonly_fields
                    ]
                    target_child_table_grid_doctype = child_table_readonly_fields[
                        0
                    ].target_child_table_doctype
                    # readonly_filter = [readonly_fields[0].parent_doctype, 'name', '=', frappe.web_form.get_value(readonly_fields[0].parent_field)]
                    script = """
                    const child_table_readonly_fields={readonly_fields};
                    // reset the dependent values
                    for (var i = 0; i < child_table_readonly_fields.length; i++) {{  
                        const target_field = readonly_fields[i].target_field;
                        if(child_table_readonly_fields[i].is_target_a_child_table) {{
                            frappe.web_form.set_value(target_field, []);
                        }} else {{
                            frappe.web_form.set_value(target_field, '');
                        }}						
                        ...
                        parent_doctype=doctype,
                        parent_field=parent_field,
                        readonly_fields=child_table_readonly_fields,
                    )

                    if child_table_readonly_fields[0].is_target_a_child_table:
                        script += """
                            frappe.web_form.doc.{target_field} = []; // rest as we wait to load from backend
                            frappe.web_form.doc.{target_field}.splice(0);
                            const web_form_values = frappe.web_form.get_values(true, false);
                ...
                if len(self.link_filters_map) > 0 or len(self.read_only_fields_map) > 0:
                    ...
                    for target in target_fields:
                        engagement_form_field = [
                            x for x in self.form_fields if x.field_name == target
                        ]
                        final_field_filters = []
                        # get the filters
                        filters = [
                            x for x in self.link_filters_map if x.target_field == target
                        ]
                        for filter in filters:
                            if filter.depends_on_form_field_value:
                                filter_val = filter.filters_parsed[
                                    3
                                ]  # filter is of the form [['Admin 2', 'parent_admin', '=', 'doc.admin_1']]
                                filter_val = filter_val.replace(
                                    DOC_PREFIX_FORMULA, "web_form_values."
                                )
                                final_field_filters.append(
                                    [
                                        filter.filters_parsed[0],
                                        filter.filters_parsed[1],
                                        filter.filters_parsed[2],
                                        filter_val,
                                    ]
                                )
                            else:
                        ...
                        func_name, func_script = _make_target_field_function(
                            engagement_form_field[0], filters=final_field_filters
                        )
                    ...
                    if REPLACE_READONLY_FIELDTYPE_WITH_ORIGINAL_FIELDTYPE:
                        # If we are replacing the ReadOnly fieldtype with the original fieldtype, then include all of them
                        read_only_source_fields += [
                            x.parent_field
                            for x in self.read_only_fields_map
                            if x.parent_field  # and x.is_target_a_child_table
                    ...
                    read_only_source_fields = list(set(read_only_source_fields))
                    ...
                    for source in source_fields:
                        ...
                        targets = set(
                            [
                                x.target_field
                                for x in self.link_filters_map
                                if x.source_field == source
                            ]
                        )
                    ...
                    child_table_filters = (
                        _make_filter_query_for_child_table_link_fields()
                    )
                    ...
                    if trigger_functions:
                        ...
                        field_scripts = (
                            on_load_script + field_scripts
                        )  # ensure trigger functions are triggered on_load
        ...
        def _make_target_field_function(
            ...
            filters: list[type[list[type[str]]]],
        ):
            """
        ...
        def _make_source_field_function(
            source_field_name: str,
            target_fields: list[str],
            readonly_fields_script: str,
        ):
            """
        ...
        def _make_filter_query_for_child_table_link_fields():
            ...
            for table in [x for x in self.form_fields if x.field_type == "Table"]:
                child_form = frappe.get_doc(
                    "Engagement Form", table.field_child_doctype
                )
                fields = [
                    x
                    for x in child_form.form_fields
        ...
        webform_supported_fields = (
            frappe.get_meta("Web Form Field").get_field("fieldtype").options.split("\n")
        )

        for df in [
            x
            for x in doctype.fields
            if x.fieldname not in [x.field_name for x in backend_only_fields]
        ]:  # exclude backend fields
    ...
    def create_data_protection_fields(self):
        ...
        if cint(self.show_data_processing_consent_statement):
            ...
            )  # remove the terms to ensure it always comes after the checkbox
            remove_field(
                "grant_data_processing_consent"
            )  # remove the terms to ensure it always comes before the data consent terms
...
def sanitize_web_filters(filters):
    ...
    matches = WEB_FORM_LINK_FIELD_DOC_FILTER_PATTERN.findall(
        filters
    )  # returns list of tuples as [('"', 'doc.admin_1', '"'), ('"', 'doc.admin_2', '"')]
    for match in matches:
        filterVal = match[1]

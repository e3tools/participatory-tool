...
class ReusableList(Document):
    ...
    if TYPE_CHECKING:
        ...
        from participatory_backend.engage.doctype.reusable_list_item.reusable_list_item import (
            ReusableListItem,
        )
    ...
    def validate(self):
        if self.is_new():
            pi = frappe.db.exists(
                self.doctype, {"name": ("=", self.name), "docstatus": ("<", 2)}
            )
            if pi:
                frappe.throw(
                    _("Another list with the name {0} exists").format(
    ...
    def make_doctype(self):
        ...
        def _upsert_items(doctype):
            ...
            for item in to_create:
                frappe.get_doc({"doctype": doctype, "list_name": item}).insert(
    ...
    def _set_roles(self, doc):
        ...
        for role in [
            DefaultRolesEnum.FORM_DESIGNER_USER.value,
            DefaultRolesEnum.FORM_DESIGNER_MANAGER.value,
        ]:
            r = frappe._dict()
            r["role"] = role
            r["doctype"] = "DocPerm"
